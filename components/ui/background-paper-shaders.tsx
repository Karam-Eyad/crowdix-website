...
function EnergyRing({
  ...
  return (
    <mesh ref={mesh} position={position}>
      ...
      <meshBasicMaterial color="#4F8EF7" transparent opacity={0.6} side={THREE.DoubleSide} />
    </mesh>
  )
}
...
export function BackgroundShaders() {
  return (
    <div className="fixed inset-0 w-full h-full" style={{ zIndex: -1 }}>
      ...
      >
        <ShaderPlane position={[0, 0, 0]} color1="#0a0a0f" color2="#4F8EF7" />
        <EnergyRing radius={0.55} position={[0, 0.1, -0.1]} />
        <EnergyRing radius={0.3} position={[0.6, -0.2, -0.1]} />
      </Canvas>
    </div>
  )
}
